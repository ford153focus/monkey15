class Sorter {
    constructor() {
        ...
        this.itemPriceSelector = 'dd.price__primary';
        this.itemWeightSelector = '.sku-card-small__sub-title';
        this.itemsSelector = '.catalog__grid .sku-card-small';
        this.switchSorting();
        this.loadMoreInterval = setInterval(() => { this.loadMore(); }, 1530);
    }
    switchSorting() {
        let switcher = document.querySelector('.catalog__sorting-dropdown .dropdown__label');
        let targetButton = document.querySelector('a[data-value="ByPriceAsc"]');
    ...
    getPrice(item) {
        ...
        let priceTxt = priceElmnt.innerText.replace(',', '.').replace(/\D/g, '');
        let price = parseFloat(priceTxt);
        return price;
    }
    getWeight(item) {
        let weightElmnt = item.querySelector(this.itemWeightSelector);
        let weightTxt = weightElmnt.innerText;
